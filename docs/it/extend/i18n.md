...
Il tuo istinto potrebbe essere quello di aggiungere un'unica traduzione &mdash; chiuamiamola "`edit_stuff`" &mdash; e usiamo quella chiave ID due volte nel tuo codice. Questo approccio è efficiente, ma manca di flessibilità: in alcune lingue, potrebbe non essere possibile utilizzare la stessa frase sia per il pulsante che per il titolo della finestra di dialogo! Un modo migliore sarebbe definire _ due _ chiavi da utilizzare nel codice, quindi impostarle entrambe per fare riferimento alla stessa traduzione, in questo modo:
...
Tutte le traduzioni devono essere organizzate in categorie, utilizzando chiavi di spaziatura dei nomi disposte in un massimo di ** tre ** livelli. Ogni livello fornisce ai localizzatori un bit importante di informazioni su _ dove viene utilizzata la traduzione: _

#### ➡ La chiave di primo livello indica _ quale componente utilizza la traduzione _.
...
#### ➡ La chiave di secondo livello indica _ quale interfaccia utilizza la traduzione _.
...
#### ➡ La chiave di terzo livello indica _ quale parte dell'interfaccia utente utilizza la traduzione _.
...
#### ➡ Il suffisso indica _ come viene utilizzata la traduzione _.
...
#### ➡ La radice indica _ cosa dice la traduzione _.
...
In generale, qualsiasi testo visualizzato che non è fornito da una variabile o il risultato di un calcolo _ deve _ essere incluso nei file delle impostazioni locali. È facile a dirsi, ma in realtà per farlo ci vuole un po' di perseveranza.
...
Se le chiavi di spaziatura dei nomi si combinano per formare una specifica completa di dove viene utilizzata una traduzione, e la chiave ID specifica esattamente come viene utilizzata la traduzione e cosa dice, è ovvio che ogni chiave di traduzione completa deve essere unica. In altre parole: _ non dovresti mai usare la stessa chiave più di una volta! _
