...
### Quyền tệp
User web cần có quyền đọc và ghi các tệp và thư mục như: `vendor`, `composer.json`, `composer.lock` và `storage`. Right now a warning shows up when this is not the case, this should preferably be changed to mention only the files/dirs where permissions are lacking instead of all of them.
