...
Vedi [Symfony documenti sulle Traduzioni](https://symfony.com/doc/5.2/translation/message_format.html) per esempi più complessi.
...
Innanzitutto, dovrai ottenere un'istanza della classe `Flarum\Locale\Translator`. Di solito lo implementerai digitando questa classe nel costruttore in modo che un'istanza del traduttore venga iniettata dal [IoC Container](https://laravel.com/docs/8.x/container). Se la relativa dipendenza non è disponibile, è possibile utilizzare `resolve(Translator::class)`. Non dimenticare l'istruzione `use Flarum\Locale\Translator;` in cima al tuo file PHP!
...
Questa è in pratica una breve sintesi del contenuto della traduzione. Se la traduzione è una frase molto breve, non più lunga di poche parole, potresti volerla usare alla lettera (in `snake_case`, naturalmente). Se è molto lungo, invece, dovresti provare a ridurla nel minor numero di parole possibile.
...
Potresti anche voler aggiungere **commenti** dopo una traduzione specifica per fornire ad altri sviluppatori ulteriori informazioni su quella traduzione (come il fatto che una traduzione può essere pluralizzata se necessario).
...
Poiché ci sembrava possibile che un traduttore avesse bisogno di questo tipo di flessibilità, abbiamo optato per "astrarre" la riga per una singola traduzione, utilizzando tag in stile HTML per racchiudere il testo del collegamento:
...
&hellip; ma sarebbe un ottima idea aggiungere un commento dopo questa traduzione, per avvertire chi stà traducendo che è possibile aggiungere altre verianti, se fossero necessarie.
