...
有兴趣为 Flarum 的发展做贡献吗？竭诚欢迎，[报告错误](bugs.md) 或 Pull Request 都没问题！

贡献前，请仔细阅读 [行为准则](code-of-conduct.md)。

此页是为想要向 Flarum 贡献代码的开发者提供的，如果您刚刚入门，建议阅读进阶文档中的 [起步](/extend/start.md) 了解 Flarum 的工作原理。

## 如何开始

请查看我们的规划 [里程碑](https://github.com/flarum/core/milestones)，了解一下需要做的事情。您可以查看[「Good first issue」](https://github.com/flarum/core/labels/Good%20first%20issue)标签中的问题，这些问题都比较容易上手。

如果您着手进行某项工作，请先在相关问题中发表评论或创建一个新的问题告知我们，以免做无用功。
...
git clone https://github.com/<username>/tags.git packages/tags # 以此类推
```

接着，将 `composer.json` 中的 `minimum-stability` 改为 `dev`，以使 Composer 接受本地副本中的不稳定包版本。

最后，运行 `composer install` 从本地路径存储库完成扩展程序安装。
...
4. 💾 **提交代码**，并附上一条描述性信息。
    ...
    * 撰写一个 [优秀的 commit message](https://tbaggery.com/2008/04/19/a-note-about-git-commit-messages.html)。
...
我们使用 [标准的键名格式（尚未翻译）](https://docs.flarum.org/extend/i18n.html#appendix-a-standard-key-format) 来确保翻译键以一致的方式被准确命名。
