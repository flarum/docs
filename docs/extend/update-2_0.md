...
* Console commands can now implement Laravel's `Illuminate\Contracts\Console\Isolatable` interface, which adds an `--isolated` option that prevents the command from running when another instance of it is already running. Core's `migrate` command implements this — useful for multi-server or multi-container deployments where several instances may try to migrate at once. See the [console documentation](console.md#running-commands-in-isolation) for details.
