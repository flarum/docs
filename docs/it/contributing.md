...
Interessato a contribuire allo sviluppo di Flarum? » fantastico! Dalla [segnalazione di un bug](bugs.md) alla creazione di richieste particolari: ogni contributo Ë apprezzato e utile.

Prima di contribuire, leggi il [codice di condotta](code-of-conduct.md).

Questo documento Ë una guida per gli sviluppatori che vogliono contribuire con codice a Flarum. Se hai appena iniziato, ti consigliamo di leggere la documentazione [Getting Started](/extend/start.md)  (per ora in inglese) sul funzionamento di Flarum.

## Su cosa lavorare

Dai un occhiata ai nostri prossimi [Traduardi (in inglese)](https://github.com/flarum/core/milestones) per una panoramica di ciÚ che deve essere fatto. Consulta le [Good first issue](https://github.com/flarum/core/labels/Good%20first%20issue) per un elenco di problemi con cui dovrebbe essere relativamente facile iniziare.
...
Fatto ciÚ la tua installazione locale Ë impostata, assicurati di abilitare la modalit‡ `debug` in **config.php**, e imposta `display_errors` su `On` nella tua configurazione php. Questo ti permetter‡ di vedere i dettagli dell'errore sia per Flarum che per PHP. La modalit‡ di debug impone anche una ricompilazione dei file delle risorse di Flarum su ogni richiesta, eliminando la necessit‡ di chiamare `php flarum cache:clear` dopo ogni modifica al javascript o CSS dell'estensione.

Il codice front-end di Flarum Ë scritto in ES6 e trasferito in JavaScript. Durante lo sviluppo sar‡ necessario ricompilare JavaScript utilizzando [Node.js](https://nodejs.org/). **Please do not commit the resulting `dist` files when sending PRs**; this is automatically taken care of when changes are merged into the `master` branch.
...
Il processo Ë lo stesso per le estensioni, tranne per il fatto che devi collegare il JavaScript principale all'estensione in modo che il tuo IDE possa capire le dichiarazioni.
...
Un esempio tipico di flusso di lavoro puÚ essere questo:

1. üå≥ **Diramare** il ramo appropriato in un nuovo ramo di funzionalit‡.
    * * Le correzioni di bug* dovrebbero essere inviate all'ultimo ramo stabile.
    * *Funzionalit‡ minori* che sono completamente retrocompatibili con l'attuale versione di Flarum possono essere inviate all'ultimo ramo stabile.
    * *Funzionalit‡ importanti* fdevono essere inviate nel ramo `master` che contiene la successiva versione di Flarum.
    * Internamente usiamo lo schema di denominazione `<initials>/<short-description>` (es. `tz/refactor-frontend`).

2. üî® **Scrivi** un po di codice.
    * Guarda qui per il [Coding Style](#coding-style).

1. üö¶ **Testa** il tuo codice.
    * Aggiungi unit tests in base alle esigenze durante la correzione di bug o l'aggiunta di funzionalit‡.
    * Lancia la suite di test con `vendor/bin/phpunit` nella cartella del pacchetto pertinente. 
<!--
    * Guarda [qui](link-to-core/tests/README.md) per maggiori informazioni su come testare Flarum.
-->

4. üíæ **Salva** il tuo codice con un messaggio descrittivo.
    * Se la modifica risolve un problema esistente (di solito, dovrebbe) includere "Fixes #123" in una nuova riga, dove 123 Ë il numero del problema.
...
5. üéÅ **Invia** una Pull Request su GitHub.
    ...
    * Se la modifica Ë visiva, includi uno screenshot o una GIF che dimostri la modifica.
...
7. üï∫ **Balla** come se avessi contribuito a Flarum.
...
Non preoccuparti se lo stile del tuo codice non Ë perfetto! StyleCI unir‡ automaticamente tutte le correzioni di stile nei repository Flarum dopo l'unione delle richieste pull. Questo ci permette di concentrarci sul contenuto del contributo e non sullo stile del codice.

### PHP

Flarum segue gli standard di codice [PSR-2](https://github.com/php-fig/fig-standards/blob/master/accepted/PSR-2-coding-style-guide.md) e [PSR-4](https://github.com/php-fig/fig-standards/blob/master/accepted/PSR-4-autoloader.md). Inoltre, ci conformiamo a una serie di  [altre regole di stile](https://github.com/flarum/core/blob/master/.styleci.yml). Usiamo suggerimenti di tipo PHP 7 e dichiarazioni di tipo restituito ove possibile, e [PHPDoc](https://docs.phpdoc.org/) fornire documentazione in linea. Prova a imitare lo stile utilizzato dal resto del codice nei tuoi contributi.
...
* Chiave esterna:  `{verbed}_{entity}_id` (es. hidden_user_id)
    * Il verbo puÚ essere omesso per la relazione primaria (es. autore del post Ë semplicemente `user_id`)
* BOOL: `is_{adjective}` (es. is_locked)

**Le Tabelle** dovrebbero chiamarsi in questo modo:
* Usa la forma plurale (`discussions`)
* Separa pi˘ parole con il trattino basso (`access_tokens`)
...
Le classi CSS di Flarum seguono pi˘ o meno il [SUIT CSS naming conventions](https://github.com/suitcss/suit/blob/master/doc/naming-conventions.md) con il formato  `.ComponentName-descendentName--modifierName`.
...
Per il forum in locale, [Laravel Valet](https://laravel.com/docs/master/valet) (Mac), [XAMPP](https://www.apachefriends.org/index.html) (Windows), e [Docker-Flarum](https://github.com/mondediefr/docker-flarum) (Linux) sono le scelte pi˘ popolari.
...
Dichiari che i Contributi sono tue opere d'autore originali e, per tua conoscenza, nessun'altra persona puÚ rivendicare, o ha il diritto di rivendicare, alcun diritto su qualsiasi invenzione o brevetto relativo ai Contributi. Dichiari inoltre di non essere legalmente obbligato, sia stipulando un contratto che in altro modo, in qualsiasi circostanza che sia in conflitto con i termini di questa licenza.

La Fondazione Flarum riconosce che, ad eccezione di quanto esplicitamente descritto nel presente Accordo, qualsiasi Contributo fornito Ë "COSÃ COM'»", SENZA GARANZIE O CONDIZIONI DI ALCUN TIPO, ESPLICITE O IMPLICITE, INCLUSE, SENZA LIMITAZIONE, ALCUNA GARANZIA O CONDIZIONE DI TITOLO, NON VIOLAZIONE, COMMERCIABILIT¿ O IDONEIT¿ PER UN PARTICOLARE SCOPO.