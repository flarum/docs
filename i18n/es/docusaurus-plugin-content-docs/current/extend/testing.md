...
- El método `prepareDatabase()` le permite pre-poblar su base de datos. Esto podría incluir la adición de usuarios, discusiones, mensajes, configuración de permisos, etc. Its argument is an associative array that maps table names to arrays of [record arrays](https://laravel.com/docs/11.x/queries#insert-statements).
...
To negate any of these methods, simply prefix them with `not.`. Por ejemplo:
