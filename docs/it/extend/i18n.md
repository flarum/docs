...
This example is from the [Choose Tags modal](https://github.com/flarum/tags/blob/master/js/src/forum/components/TagDiscussionModal.js) of the Tags extension, where it tells the user how many more primary tags can be selected. Nota che la variabile `remaining` viene passato al traduttore ** due volte **. In primo luogo, sembra lo stesso, per condizionare la pluralizzazione della parola "tag". Quindi appare di nuovo come valore del parametro `count`, che il traduttore può utilizzare per inserire quel valore nella traduzione.
...
For this reason, any key references in the [core resources](https://github.com/flarum/core/blob/master/locale/core.yml) **must** point to keys in the `core.ref` namespace. I riferimenti chiave nelle risorse per le estensioni in bundle possono puntare a una delle due posizioni:
...
E la situazione è complicata dalla presenza del pronome in seconda persona, dato che in inglese assume sempre la forma plurale, anche quando si parla di una persona. That's why the `app.translator` call is so complicated in the code that outputs the above sentences for the [Likes extension](https://github.com/flarum/likes/blob/master/js/src/forum/addLikesList.js).
