...
Flarum保存在一个中间件“管道”，所有请求都可以通过它。 Flarum maintains a middleware "Pipe" through which all requests pass. Each of the three "applications" (`admin`, `forum`, and `api`) have their own subpipe: after being processed through some shared logic, requests are diverted to one of the pipes based on the path.
...
要添加一个新的中间程序，只需在扩展名的 `extend.php` 文件中使用中间件扩展程序：
...
如果您不需要在每个路由中都执行中间件，您可以添加`if`过滤：
...
要了解更多关于请求和响应对象的信息，请参阅 [PSR HTTP消息接口](https://www.php-fig.org/psr/psr-7/#1-specification) 文档。
...
要记住，PSR-7 响应是不可变的，因此你每次修改响应后，都需要重新为 `$response` 变量赋值。
